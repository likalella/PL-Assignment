...
public:
	...
	int Minimize(struct treeNode* root, int alpha, int beta);		/* Eval 값중 Min 값을 구함 */
	int Maximize(struct treeNode* root, int alpha, int beta);		/* Eval 값중 Max 값을 구함 */
	int CheckSymmetric(GameBoard preBoard, GameBoard newBoard);	/* 게임보드 대칭성 검사 */
	int CheckSameBoard(char(*board1)[4], char(*board2)[4]);	/* 대칭성 검사 중 보드가 일치하는지 검사 */ // lika : 3->4
